...
//  PRIVILEGED_DATA static xList pxAllTasksLists[ configMAX_PRIORITIES ];
...
unsigned portBASE_TYPE xTaskHeadListPriority( const xList * const pxEventList )
{
...
unsigned portBASE_TYPE xReturn;
	...
	/* The event list is sorted in priority order, so we can peek at the
	first in the.
  ...
  */
  ...
  xReturn = pxUnblockedTCB->uxPriority ;
...
//	for( uxPriority = ( unsigned portBASE_TYPE ) 0U; uxPriority < configMAX_PRIORITIES; uxPriority++ )
//	{
//		vListInitialise( ( xList * ) &( pxAllTasksLists[ uxPriority ] ) );
//	}
//  
//  #endif
...
#if ( ( INCLUDE_xTaskGetCurrentTaskHandle == 1 ) || ( configUSE_MUTEXES == 1 ) || ( configUSE_PCP == 1 ) )
...
// xReturn returns the current priority if there are no resources with 
// tasks blocked with priority higher than the current task  
  
unsigned portBASE_TYPE xGetHighestPriorityTasksBlocked( xTaskHandle * const pxPcpHolder )
{
  unsigned portBASE_TYPE xReturn  = tskIDLE_PRIORITY; 
  unsigned portBASE_TYPE priority = uxTaskPriorityGet( pxPcpHolder );
  ...
  /* The pxPcpHolder task will inherit the higher of:
     - the blocked task (may be through indirect blocking)
     - the highest priority of the task directly blocked on resources taken by pxPcpHolder
  */
	void vTaskPcpPriorityInherit( xTaskHandle * const pxPcpHolder )
	{
    tskTCB * const pxTCB = ( tskTCB * ) pxPcpHolder;
    configASSERT( pxPcpHolder );
    unsigned portBASE_TYPE uxNewPriority = xGetHighestPriorityTasksBlocked( pxPcpHolder );
    unsigned portBASE_TYPE uxBlockedTaskPriority = pxCurrentTCB->uxPriority;
    
    if ( uxBlockedTaskPriority > uxNewPriority )
    {
      uxNewPriority = uxBlockedTaskPriority;
    }
    
		if( pxTCB->uxPriority < uxNewPriority )
		{
			/* Adjust the PCP semaphore holder state to account for its new priority. */
			listSET_LIST_ITEM_VALUE( &( pxTCB->xEventListItem ), configMAX_PRIORITIES - ( portTickType ) uxNewPriority );
			...
			traceTASK_PRIORITY_INHERIT( pxTCB, uxNewPriority );
		}
	}

unsigned portBASE_TYPE xTestHigherPriorityCeiling( xTaskHandle * const pxPcpHolder )
{
  unsigned portBASE_TYPE xReturn = tskIDLE_PRIORITY;
  unsigned portBASE_TYPE res_idx;
  unsigned portBASE_TYPE priority = uxTaskPriorityGet( pxPcpHolder );
  ...
  for ( res_idx = 0; res_idx < uRes; res_idx++ )
  {
    if ( resources[res_idx].status            == ACTIVE &&
         resources[res_idx].pcHoldingTask     == (signed char *) pxPcpHolder &&
         resources[res_idx].uxPriorityCeiling >= priority 
       )
    {
      priority = resources[res_idx].uxPriorityCeiling;
      xReturn  = priority;
    }
  ...
  return xReturn;
	...
	void vTaskPcpPriorityDisinherit( xTaskHandle * const pxPcpHolder )
	{
	  tskTCB * const pxTCB = ( tskTCB * ) pxPcpHolder;
    unsigned portBASE_TYPE uxNewPriority = xGetHighestPriorityTasksBlocked( pxPcpHolder );

    /* If no other tasks blocked on this task, keep base priority */
    if ( uxNewPriority == tskIDLE_PRIORITY )
    {
      uxNewPriority = pxTCB->uxBasePriority;
    }
    
		if( pxPcpHolder != NULL )
		{
      /* Check if the task has inherited any priority */
			if( pxTCB->uxPriority != pxTCB->uxBasePriority )
			{

        /* Check if the holder task has released every resource whose priority ceiling is
           greater than or equal to the inherited priority
        */
        if ( xTestHigherPriorityCeiling(pxPcpHolder) == 0 )
        {
        ...
        if( pxTCB->uxPriority != uxNewPriority )
        {
          /* We must be the running task to be able to give the PCP semaphore back.
          Remove ourselves from the ready list we currently appear in. */
          vListRemove( &( pxTCB->xGenericListItem ) );

          /* Disinherit the priority before adding the task into the new ready list. */
          traceTASK_PRIORITY_DISINHERIT( pxTCB, uxNewPriority );
